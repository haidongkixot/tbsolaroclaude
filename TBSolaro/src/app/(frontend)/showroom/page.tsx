...
export default function ShowroomPage() {
  ...
  return (
    <>
      <PageHero
        title="Showroom"
        subtitle="Ghé thăm showroom TBSolaro để trải nghiệm trực tiếp các sản phẩm và được tư vấn miễn phí"
        backgroundImage="https://placehold.co/1600x500/1B5E30/FFFFFF?text=Showroom"
        size="md"
      />
      ...
      <section className="py-12 md:py-16">
        <div className="container-site">
          <h2 className="section-title mb-6">Địa chỉ các Showroom</h2>
              ...
              <div className="w-full h-[500px] rounded-2xl overflow-hidden bg-gray-200 border border-gray-200 flex items-center justify-center">
                <div className="text-center text-gray-500">
                  ...
                  <p className="text-sm mt-1">
                    {showrooms.find((s) => s.id === selected)?.name || ''}
                  </p>
                  <p className="text-xs mt-1 text-gray-400">
                    {showrooms.find((s) => s.id === selected)?.address || ''}
                  </p>
      ...
      <section className="py-12 bg-gray-50">
        <div className="container-site">
          <div className="text-center mb-10">
            <h2 className="section-title">Các hoạt động tại Showroom</h2>
            <p className="section-subtitle">Hình ảnh người dùng trải nghiệm dịch vụ, hoạt động tại mỗi showroom</p>
          </div>
          ...
          <div className="text-center mt-8">
            <button className="btn-primary">Xem thêm ảnh</button>
          </div>
        </div>
      </section>
      ...
      <section className="py-16">
        <div className="container-site">
          <div className="grid lg:grid-cols-2 gap-10 items-start">
            <div>
              <h2 className="text-3xl font-bold text-gray-900 mb-4">Đăng ký tham quan showroom</h2>
              <p className="text-gray-600 mb-6 leading-relaxed">
                Đăng ký để được tư vấn trực tiếp tại showroom gần bạn nhất.
                Đội ngũ chuyên gia TBSolaro sẽ giúp bạn tìm giải pháp phù hợp nhất.
              </p>
              <div className="bg-brand-surface rounded-2xl p-6 border border-brand/10">
                <p className="text-sm text-gray-700 font-medium mb-3">Lợi ích khi tham quan showroom:</p>
                <ul className="space-y-2">
                  {[
                    'Trải nghiệm trực tiếp sản phẩm',
                    'Tư vấn miễn phí từ chuyên gia',
                    'Nhận báo giá chi tiết tại chỗ',
                    'Xem demo hệ thống thực tế',
                  ].map((benefit) => (
            ...
            <div>
              {submitted ? (
                <div className="text-center py-10">
                  ...
                  <p className="text-gray-600 text-sm">Chúng tôi sẽ liên hệ xác nhận lịch tham quan trong thời gian sớm nhất.</p>
                  <button onClick={() => setSubmitted(false)} className="mt-6 btn-primary">Đăng ký thêm</button>
                </div>
              ) : (
                <form onSubmit={handleBooking} className="space-y-4">
                  <div className="grid sm:grid-cols-2 gap-4">
                    <div>
                      ...
                      <input required value={bookingForm.name} onChange={(e) => setBookingForm((f) => ({ ...f, name: e.target.value }))} placeholder="Họ và tên" className="input-field" />
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <input value={bookingForm.area} onChange={(e) => setBookingForm((f) => ({ ...f, area: e.target.value }))} placeholder="Khu vực của bạn" className="input-field" />
                    </div>
                  </div>
                  <div>
                  ...
                  <button type="submit" disabled={submitting} className="btn-primary w-full justify-center disabled:opacity-60">
                    {submitting ? 'Đang gửi...' : 'Đặt lịch ngay – Tư vấn miễn phí tại showroom'}
                  </button>
                </form>
              )}
            </div>
